...
const outputPath = path.join(outputDir, 'readme-graph.png')

const workspaceMarkdown = `# LLM systems map
...
async function main() {
  ...
  await page.addInitScript((markdown) => {
    window.localStorage.setItem('vibe-mind.workspace', markdown)
  }, workspaceMarkdown)
  ...
  await writeFile(
    path.join(outputDir, 'readme-graph.md'),
    workspaceMarkdown,
    'utf8',
  )
